...
spi['vwap_band_upper'] = spi['vwap'] + (1 * spi['vwap']*(0.333333/100))
spi['vwap_band_lower'] = spi['vwap'] - (1 * spi['vwap']*(0.333333/100))
...
mpf.plot(spi, type='candle', volume=True, title='Candlestick Chart with VWAP', ylabel='Price',
         addplot=[
            mpf.make_addplot(spi['vwap'], color='blue', panel=0), 
             mpf.make_addplot(spi['vwap_band_upper'], color='green', panel=0, linestyle='--'),
             mpf.make_addplot(spi['vwap_band_lower'], color='red', panel=0, linestyle='--')
         ])
